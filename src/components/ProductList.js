...
export default function ProductList() {
    ...
    async function handleFiltered(filtered) {
    ...
    return (
        <div className="mt-4 space-y-2">
            <div className='flex flex-row gap-4'>
                <button
                    className={`flex justify-center items-center w-1/2 h-12 rounded shadow-md p-4 ${!isFiltered ? "bg-blue-200" : "bg-white"}`}
                    onClick={() => { handleFiltered(false) }}>TÜM ÜRÜNLER</button>
                <button
                    className={`flex justify-center items-center w-1/2 h-12 rounded shadow-md p-4 ${isFiltered ? "bg-blue-200" : "bg-white"}`}
                    onClick={() => handleFiltered(true)}>SEPETTEKİLER</button>
            </div>
                ...
                {isLoading ? <Loading width={10} height={10} /> :
                    products.length == 0 ? <div className='text-center'>Ürün Bulunamadı</div> :
                        products.map((product) => {
                            if (!isFiltered || (isFiltered && product.is_required))
